...
- 🔐 **多用户访问鉴权** —— 超级管理员由 `HME_SUPERADMIN_USERNAME` / `HME_SUPERADMIN_PASSWORD` 配置，普通用户只能由管理员创建
...
```bash
export HME_SUPERADMIN_USERNAME='liuyuquan'
export HME_SUPERADMIN_PASSWORD='请替换为强密码'
docker compose up -d --build
```
...
HME_SUPERADMIN_USERNAME='liuyuquan' HME_SUPERADMIN_PASSWORD='请替换为强密码' ./icloud_distribution
```
...
- 超级管理员凭据仅从 `HME_SUPERADMIN_USERNAME` / `HME_SUPERADMIN_PASSWORD` 环境变量读取；普通用户存储在 SQLite，密码使用 Argon2id 哈希，会话在密码修改或停用后立即失效
