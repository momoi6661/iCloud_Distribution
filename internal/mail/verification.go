...
var (
	...
	verificationKeywordPrefix      = regexp.MustCompile(`(?i)(?:验证码|校验码|动态码|一次性(?:验证码|代码)|安全码|安全代码|verification(?:\s+code)?|security(?:\s+code)?|one[-\s]?time\s+code|passcode|otp|\bcode\b)\s*(?:is|为|是|的)?\s*[:：=\-]?\s*((?:[0-9][ \t-]*){3,7}[0-9])`)
	verificationKeywordSuffix      = regexp.MustCompile(`(?i)((?:[0-9][ \t-]*){3,7}[0-9])\s*(?:是你的验证码|为你的验证码|is your code|is your verification code)`)
	verificationChineseInstruction = regexp.MustCompile(`(?i)(?:验证码|校验码|动态码|安全码|安全代码)[^\r\n:：=]{0,16}[:：=]\s*((?:[0-9][ \t-]*){3,7}[0-9])`)

	// Verification codes are intentionally numeric-only. This avoids consuming
	// the beginning of a following word, such as "432191 Please".
	digitCandidatePattern = regexp.MustCompile(`(?i)(^|[^A-Z0-9])((?:[0-9][ \t-]?){3,7}[0-9])([^A-Z0-9]|$)`)
	yearCode              = regexp.MustCompile(`^20[2-3]\d$`)
	repeatedDigitCode     = regexp.MustCompile(`^(0{4,8}|1{4,8}|2{4,8}|3{4,8}|4{4,8}|5{4,8}|6{4,8}|7{4,8}|8{4,8}|9{4,8})$`)
...
// ExtractVerificationCode selects a likely numeric OTP by shape and context.
// Language words are optional evidence, not a gate.
func ExtractVerificationCode(text string) string {
	...
	for _, candidate := range best {
		// Very short bare identifiers still need some context.
...
func normalizeVerificationCode(value string) string {
	...
	if yearCode.MatchString(code) || repeatedDigitCode.MatchString(code) {
		return ""
	}
	return code
}
