...
export default function UsersPage({ onLogout }: { onLogout: () => void }) {
  ...
  const reset=async(user:AppUser)=>{const password=window.prompt(`为 ${user.username} 设置新密码（至少 8 位）`);if(!password)return;try{await api.resetUserPassword(user.id,password,true);setNotice('密码已重置，该用户现有会话已失效。');await load()}catch(e){setNotice((e as Error).message)}}
